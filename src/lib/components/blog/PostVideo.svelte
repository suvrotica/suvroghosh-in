<script lang="ts">
	...
	let fullSrc = $derived(src.startsWith('/') || src.startsWith('http') ? src : `/videos/${src}`);

	let fullPoster = $derived(
		poster
			? poster.startsWith('/') || poster.startsWith('http')
				? poster
				: `/images/${poster}`
			: undefined
	);
	...
	let videoClasses = $derived(
		'h-auto w-full rounded-lg shadow-md border border-neutral-200 dark:border-neutral-800 bg-black object-cover'
	);
</script>
