...
thumbnailAlt: "A mature solver-generated Oregonator spiral curling through a circular luminous dish beside the Belousov–Zhabotinsky Laboratory title"
...
> **Start here:** the Gallery opens on the validated **Persistent Single Spiral** at its mature checkpoint. In Gentle or Alive motion mode it continues from that exact numerical state; in Still mode it waits, intact, for **Play** or **Step**. Switch display styles before opening the Laboratory—the wave should not move merely because its colour mapping changed.

<BZExperienceV2 />

## One field, two views

**Scientific** view shows the dimensionless model fields on fixed ranges. **Ferroin-inspired** view is a representative colour mapping, not a measurement of ferroin or ferriin concentration. **Luminous** view combines a fixed phase coordinate with measured front information so that narrow waves remain legible. Interpolation, exposure, bloom, tone mapping and the glass rim affect presentation only. No ring, core or spiral is generated by the renderer.

### What is physics, and what is presentation?

| Numerical model | Display only |
|---|---|
| Oregonator or Schnakenberg reaction terms | Mask-aware visual interpolation |
| Diffusion coefficients and five-point Laplacian | Palette and representative colour |
| Fixed timestep and Heun integration | Exposure and saturation |
| No-flux or periodic boundary | Front-linked bloom |
| Initial numerical state | ACES tone mapping and sRGB transfer |
| Declared pacemaker schedule | Glass rim and meniscus cue |
| Domain and obstacle masks | Labels and explanatory overlays |

Changing the right-hand column cannot reset or alter the left-hand state. The Proof layer exposes the validation record, checkpoint provenance, convergence comparisons and finite-time claim boundaries behind each Gallery badge.
...
The well-mixed control keeps every active cell identical, so its spatial variance remains at round-off scale while the local state changes. It is a control for distinguishing a temporal clock from a travelling wave; the three validated Gallery claims belong to separate spatial presets and their declared finite observation windows.

The well-mixed control makes the distinction visible. Every numerical cell begins identically, so diffusion has nothing to transport. The dish changes as one, while the local $u(t)$ and $v(t)$ traces loop around the phase portrait. Pause on two moments with similar colour and inspect both fields: a display colour can conceal that the full chemical state differs. That is why one coloured channel is a poor substitute for phase space.
...
A target pattern needs a source that fires repeatedly. In this model the validated source is a declared local state-reset intervention with period 1.5 model-time units, active from $t=0$ through $t=60$. Each event can launch a radial front into recovered territory. Because the circumference grows as the front expands, a ring becomes geometrically larger without material being carried outward as a hoop.
...
**In plain language:** a faster front or a slower clock puts rings farther apart. The relation is an operational wave estimate, not a promise that curvature, refractory variation and finite boundaries leave $c$ perfectly constant.

For the promoted 256² target run, the declared $t=10$ to $t=14$ window contained three significant radial peaks in 96.3% of samples. The minimum measured spacing was 12.506 dimensionless length units, four tracks moved outward with mean speed 4.866, peak order was preserved, and no reflected wall track was counted as a new wave. The 128² refinement and timestep-halved controls preserved the repeated outward-front result.

<Pi
	src="/images/visualizations/belousov-zhabotinsky/v2/classic-target-rings-plate.png"
	alt="A mature 512-square-cell Oregonator state with several clean concentric excitation fronts travelling outwards from a declared central repeating source in a circular no-flux dish"
	caption="Classic Target Rings at t = 14. The 512² publication state uses the exact promoted parameters and source schedule; colour, interpolation, restrained bloom and glass are display-only."
/>
...
The promoted single-spiral preparation starts with a plane wave and performs one declared half-plane reset at $t=0.45$, leaving a free end rather than painting a curved arm. From $t=2$ to $t=14$, one same-charge phase-winding core was tracked at every sample. The annular front orientation accumulated 4.210 rotations, its mean measured period was 2.7372 with coefficient of variation 0.00455, and the core retained at least 8.161 dimensionless units of wall clearance. The 128² and 96² refinements and the timestep-halved run preserved the core and approximate period.

<Pi
	src="/images/visualizations/belousov-zhabotinsky/v2/persistent-single-spiral-plate.png"
	alt="A mature solver-generated Oregonator spiral with one tracked interior phase core and a long curling excitation front inside a circular no-flux dish"
	caption="Persistent Single Spiral at t = 14, rendered from the 512² Float64 state. The measured core and rotations come from raw numerical fields, not from this luminous mapping."
/>
...
The third Gallery regime begins from three declared phase singularities. Across $t=2$ to $t=12$, three continuously associated cores persisted for the full ten-unit window; the least-rotating track completed 3.621 rotations, minimum pairwise separation was 5.216, and minimum wall clearance was 6.933. A 96² refinement and a timestep-halved control preserved the three-core result.

<Pi
	src="/images/visualizations/belousov-zhabotinsky/v2/spiral-garden-plate.png"
	alt="A mature solver-generated Oregonator field with three separated curling spiral cores interacting inside one circular no-flux dish"
	caption="Spiral Garden at t = 12. Three persistent cores are a tracked finite-time result; the luminous palette only reveals the state already present in u and v."
/>
...
The collision control launches opposing fronts and records active length before and after contact. If they pass through and continue unchanged, the explanation has failed—or the model is in a different regime. Annihilation is not a display fade; it is a consequence of local state.
...
**In plain language:** the $u^2v$ term transfers local influence between the fields with opposite signs, $a$ and $b$ supply the selected kinetics, and $\gamma$ sets their overall timescale. At $(u_*,v_*)$ both reaction terms vanish. Start **Stable uniform** to test whether numerical noise remains quiet; then choose the finite-mode Turing comparator and inspect its dispersion status before calling the mature field a spot, stripe or labyrinth. These are dimensionless comparator fields, not named morphogens.
...
<Pi
	...
	alt="A mature rotating Oregonator spiral beside a solver-generated Schnakenberg field whose reaction-stable equilibrium has a resolved non-zero growing diffusion band"
	caption="Two real 512² solver states, different diagnostics: the BZ front rotates around a tracked phase core; the Schnakenberg comparator is classified from its stable zero mode and growing finite-wave-number band. Visual resemblance alone proves neither mechanism."
/>
...
**Inspect the pacemaker.** Select **Classic Target Rings** and record successive crossings at one radius. Support: each crest reaches the probe later than the previous radial location and the measured spacing is compatible with speed × period. Contradiction: rings flash everywhere at once.
...
**Collide two waves.** Launch opposing fronts as an exploratory Laboratory run. Support: both disappear at contact because the encounter leaves no recovered continuation. Contradiction: the fronts pass through with unchanged identity as linear ripples would. This is not one of the three promoted Gallery claims unless its own recorded criteria are later added to the manifest.
...
**Compare with the Turing model.** Run live BZ and the analytically classified Schnakenberg comparator side by side. Support: the BZ front travels while the comparator selects a finite wavelength and its inspector shows stable $k=0$ with growth at $k>0$. Contradiction: classification rests only on whether both pictures contain spots.
...
Finish with one concrete observation. In the Gallery, pause **Persistent Single Spiral** and switch between raw $u$, phase, ferroin-inspired and luminous views. The front must remain at the same numerical position. Then replay genesis: a plane wave and one declared cut must produce the evolving free tip without any renderer inventing curved geometry. The Proof layer records the finite window in which that core persisted and the surrounding front completed 4.210 measured rotations.
