...
> **Basis and scope.** This is healthcare-data architecture and causal-design analysis, not a report of one named institution or study. Unless a study is linked, the hospital, sepsis, referral, and care-management examples are composite and illustrative. “Denominator corruption” is my descriptive phrase for a recurring systems failure, not a proposed formal statistical category.
...
A deliberately simple illustration shows why the denominator deserves suspicion. These are hypothetical counts, not observed hospital data:

| Analytic view | Observed events | Eligible encounters captured | Apparent rate |
| --- | ---: | ---: | ---: |
| Complete capture | 40 | 1,000 | 4.0% |
| Same events, incomplete capture | 40 | 600 | 6.7% |

Nothing about the events changed. The conclusion bent because the capture boundary did.
...
This last category is especially poisonous in healthcare IT because it creates “smart” models that are, in fact, learning access pathways and institutional behavior rather than biology or clinical reality. A direct empirical example is the BMJ study [“Biases in electronic health record data due to processes within the healthcare system”](https://pmc.ncbi.nlm.nih.gov/articles/PMC5925441/), which showed that ordering behavior and timing can carry predictive signal beyond a test result. Its numerical findings come from two Boston hospitals in 2005–2006; the demonstrated mechanism, not a universal effect size, is what matters here.
...
## Target trials and structural missingness

Fifth, use design-based methods when the question is causal enough to require them. Multivariable regression is not the whole arsenal. Propensity score methods, inverse probability weighting, matching, marginal structural models, difference-in-differences, interrupted time series, instrumental variable approaches, target trial emulation, and hierarchical models all exist because naive comparison is usually not enough. None are magic. All can fail if assumptions are wrong. But the choice among them should be driven by the data-generating process, not by which package someone already knows how to run. The open text [*Causal Inference: What If*](https://content.sph.harvard.edu/wwwhsph/sites/1268/2024/04/hernanrobins_WhatIf_26apr24.pdf), linked here in its 26 April 2024 revision, develops the assumptions and methods in full.

In healthcare IT, [target trial emulation](https://pmc.ncbi.nlm.nih.gov/articles/PMC4832051/) is especially useful as a discipline of thought, even when not executed formally. Ask: if this were a randomized trial, who would be eligible, when would follow-up begin, what would treatment assignment mean operationally, what grace periods would be allowed, how would crossover be handled, and what outcomes would be measured when? The exercise forces you to confront confounding, immortal time, and misaligned time zero before they quietly poison the observational analysis. It clarifies design; it does not remove dependence on adequate data and defensible causal assumptions.
...
Build analytic datasets with provenance fields that survive transformation. Keep source-system identifiers, timestamps, site markers, interface lineage, versioned code mappings, and extraction dates accessible rather than hiding them under polished semantic layers. Confounding often reveals itself in lineage metadata before it reveals itself in model diagnostics. The [RECORD statement](https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.1001885) is useful here because it requires routine-data studies to expose code lists, algorithms, misclassification, linkage, and data-access limits rather than treating administrative exhaust as self-explanatory evidence.
...
STROBE and RECORD improve transparency; neither proves that a study has identified a causal effect. Target-trial emulation and DAGs clarify design, but they do not manufacture missing variables or make unjustified assumptions true.

- [Hernán and Robins, *Causal Inference: What If*, 26 April 2024 revision](https://content.sph.harvard.edu/wwwhsph/sites/1268/2024/04/hernanrobins_WhatIf_26apr24.pdf)
- [Hernán and Robins, “Using Big Data to Emulate a Target Trial”](https://pmc.ncbi.nlm.nih.gov/articles/PMC4832051/)
- [Shrier and Platt, “Reducing bias through directed acyclic graphs”](https://pmc.ncbi.nlm.nih.gov/articles/PMC2601045/)
- [STROBE explanation and elaboration](https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.0040297)
- [The RECORD statement](https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.1001885)
- [Agniel, Kohane, and Weber, “Biases in electronic health record data due to processes within the healthcare system”](https://pmc.ncbi.nlm.nih.gov/articles/PMC5925441/)
