<script lang="ts">
	...
	import {
		...
		CORPUS_VERSION,
		DIRECT_ECHO_SENTENCE_COUNT,
		ENGINE_VERSION,
		FEEDBACK_SENTENCE_COUNT,
	...
	let currentBaselineContainer: HTMLElement | undefined = $state();
	...
	let initialActionConsumed = false;
	...
	let presentedBaselineCore = $derived(
		baselineCore.filter((statement) =>
			(presentation?.presentedBaselineStatementIds ?? []).includes(statement.statementId)
		)
	);
	...
	let presentedBaselineStatements = $derived(
		toReadingStatements(presentedBaselineCore, ratingRecords)
	);
	let currentBaselineStatement = $derived(
		presentedBaselineStatements[presentedBaselineStatements.length - 1]
	);
	let baselineHistory = $derived(presentedBaselineStatements.slice(0, -1));
	let allBaselinePresented = $derived(
		baselineCore.length > 0 && presentedBaselineStatements.length === baselineCore.length
	);
	let baselineRatedCount = $derived(
		baselineStatements.filter((statement) => statement.rating !== 'unrated').length
	);
	let allBaselineRated = $derived(
		baselineStatements.length > 0 && baselineRatedCount === baselineStatements.length
	);
	let fullReadingCore = $derived([
		...genericCore.slice(0, 3),
		...(directEchoCore ? [directEchoCore] : []),
		...genericCore.slice(3)
	]);
	let fullReadingStatements = $derived(toReadingStatements(fullReadingCore, ratingRecords));
	...
	let xrayCore = $derived.by(() => {
		const unique: GeneratedStatement[] = [];
		for (const entry of presentation?.history ?? []) {
			if (!unique.some((statement) => statement.statementId === entry.statement.statementId)) {
				unique.push(entry.statement);
			}
		}
		return unique;
	});
	let xrayStatements = $derived(toReadingStatements(xrayCore, ratingRecords, presentation));
	...
	let guidedContinueLabel = $derived.by(() => {
		if (stage === 'baseline') {
			if (!allBaselinePresented) return 'Next statement';
			return allBaselineRated
				? 'Continue'
				: baselineRatedCount > 0
					? 'Continue with these ratings'
					: 'Continue without rating';
		}
		if (stage === 'apparent-sharpening') {
			return wholeReadingFit === undefined
				? 'Continue without an overall rating'
				: 'Continue to the next step';
		}
		if (stage === 'feedback-and-counterfactual') return 'See how it worked';
		return 'Continue';
	});
	...
	let abandonedGenericCount = $derived(
		genericCore.filter((statement) =>
			(presentation?.abandonedStatementIds ?? []).includes(statement.statementId)
		).length
	);
	let revealSummary = $derived.by(() => {
		const lines = ['The profile did not read your mind.'];
		if (presentedGenericCore.length) {
			lines.push(
				`${presentedGenericCore.length} original ${presentedGenericCore.length === 1 ? 'line was' : 'lines were'} shown; ${presentedGenericCore.length === 1 ? 'it was' : 'all were'} chosen before you answered.`
			);
		} else {
			lines.push('You revealed the method before any reading line was shown.');
		}
		if (abandonedGenericCount) {
			lines.push(
				`${abandonedGenericCount} prepared original ${abandonedGenericCount === 1 ? 'line was' : 'lines were'} not shown.`
			);
		}
		if (directEchoCore) lines.push('One line reworded an answer you gave.');
		if (derivativeCore.length) {
			lines.push(
				`${derivativeCore.length} ${derivativeCore.length === 1 ? 'line was' : 'lines were'} selected because of earlier ratings.`
			);
		}
		if (counterfactualApplied && counterfactualResult.identicalSemanticIds) {
			lines.push(
				`Changing the surface details left the same ${genericCore.length} original lines in the same order.`
			);
		}
		return lines;
	});
	...
	let shownDemoValuesUnconfirmed = $derived(
		(['country', 'city_context', 'language'] as const).some(
			(questionId) => answers[questionId]?.origin === 'demo-default'
		)
	);
	...
	function surfaceStageId(value: LabStage): string {
		if (value === 'baseline') return 'first-impression';
		if (value === 'four-clues') return 'a-few-clues';
		if (value === 'apparent-sharpening') return 'your-reading';
		if (value === 'feedback-and-counterfactual') return 'one-more-pass';
		if (value === 'reveal') return 'how-it-worked';
		return value;
	}

	function surfaceRatingLabel(rating: FitRating): string {
		if (rating === 'does-not-fit') return 'No, not me';
		if (rating === 'partly-fits') return 'Maybe';
		if (rating === 'fits') return 'Yes, that sounds like me';
		if (rating === 'too-vague') return 'Too broad to tell';
		return 'Not rated';
	}

	function xrayCardLabel(statement: ReadingStatement, index: number): string {
		const throughCurrent = xrayStatements.slice(0, index + 1);
		if (statement.basis === 'direct-echo') {
			const echoCount = throughCurrent.filter((item) => item.basis === 'direct-echo').length;
			return `Echo ${String.fromCharCode(64 + echoCount)}`;
		}
		if (statement.adaptation !== 'sealed') {
			const derivativeCount = throughCurrent.filter(
				(item) => item.basis !== 'direct-echo' && item.adaptation !== 'sealed'
			).length;
			return `Derivative ${String.fromCharCode(64 + derivativeCount)}`;
		}
		const originalCount = throughCurrent.filter(
			(item) => item.basis !== 'direct-echo' && item.adaptation === 'sealed'
		).length;
		return `Original ${originalCount}`;
	}
	...
	async function focusCurrentBaseline(message: string): Promise<void> {
		liveMessage = message;
		await tick();
		currentBaselineContainer?.focus({ preventScroll: true });
		currentBaselineContainer?.scrollIntoView({
			block: 'center',
			behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth'
		});
	}
	...
	function begin(): void {
		withActivationLock(() => {
			...
			if (!dispatchCore({ type: 'present-next-baseline' })) return;
			void focusStage('Step 1 of 5. Your first impression is ready.');
		});
	}
	...
	function continueForward(): void {
		withActivationLock(() => {
			...
			if (nextStage === 'apparent-sharpening') {
				for (let index = 0; index < laterCore.length; index += 1) {
					if (!dispatchCore({ type: 'present-next-claim' })) break;
				}
			}
			void focusStage(
				nextStage === 'reveal'
	...
	function advanceGuided(): void {
		if (labState.stage !== 'baseline' || allBaselinePresented) {
			continueForward();
			return;
		}
		withActivationLock(() => {
			if (!dispatchCore({ type: 'present-next-baseline' })) return;
			if (labState.stage !== 'baseline') return;
			const shown = labState.presentation.presentedBaselineStatementIds.length;
			void focusCurrentBaseline(`Line ${shown} of ${baselineCore.length} is ready.`);
		});
	}
	...
	function goBack(): void {
		if (labState.stage === 'baseline') {
			...
			void focusStage('Returned to the introduction. The current reading was discarded.');
			return;
		}
	...
	function useShownDemoValues(): void {
		let confirmed = 0;
		for (const questionId of ['country', 'city_context', 'language'] as const) {
			if (answers[questionId]?.origin !== 'demo-default') continue;
			if (dispatchCore({ type: 'confirm-default', questionId })) confirmed += 1;
		}
		liveMessage = confirmed
			? `${confirmed} shown demo ${confirmed === 1 ? 'value is' : 'values are'} now selected by you.`
			: 'The shown demo values were already selected or changed.';
	}
	...
	function rateStatement(statementId: string, rating: FitRating): void {
		...
		liveMessage = `Statement rated “${surfaceRatingLabel(rating)}.”`;
	}
	...
	function applyCounterfactual(): void {
		if (counterfactualApplied) {
			liveMessage = 'The surface-clue comparison is already shown.';
			return;
		}
		if (!dispatchCore({ type: 'apply-counterfactual' })) return;
		liveMessage = counterfactualResult.identicalSemanticIds
			? `Several surface clues changed. The same ${genericCore.length} original readings remain in the same order.`
			: 'The surface clues changed, but the unchanged-reading check failed.';
	}
	...
	function openLaboratory(experiment: OpenLabExperiment = 'demographics'): void {
		...
		if (experiment === 'demographics' && !counterfactualApplied) applyCounterfactual();
	...
	function transitionWithoutMeta(state: LabState, event: LabEvent): LabState {
		...
		if (result.ok === false) throw new Error('Core reconstruction failed: ' + result.reason);
		return result.state;
	}
	...
	function beginFreshState(nextSeed: string): LabState {
		...
		return transitionWithoutMeta(begun, { type: 'present-next-baseline' });
	}
	...
	function confirmDeckReplacement(): void {
		...
		void focusStage('Step 1 of 5. A fresh first impression is ready.');
	}
...
<section
	...
	data-stage={surfaceStageId(stage)}
...
>
	<div class="lab-inner">
		...
		<div class="lab-scroll">
			<AssumptionLedger
				rows={ledgerRows}
				revealed={stage === 'reveal' || stage === 'open-lab'}
				canUseDemoValues={stage === 'four-clues' && shownDemoValuesUnconfirmed}
				onusedemovalues={useShownDemoValues}
			/>
			...
			{:else if stage === 'open-lab'}
				<section class="open-lab" aria-labelledby="barnum-open-heading">
					...
					<OpenLabControls
						...
						counterfactualAvailable={!counterfactualApplied}
						onselfreportcounterfactual={applySelfReportCounterfactual}
						onreset={requestReset}
					/>

					{#if counterfactualApplied}
						<CounterfactualDiff result={counterfactualResult} technical />
					{/if}
					...
					{#if activeExperiment === 'hedges'}
						<HedgeComparison pair={HEDGE_PAIR} />
					{/if}
				...
				<section class="guided" aria-labelledby="guided-stage-heading">
					...
					{#if stage === 'baseline'}
						<div class="stage-heading">
							<p>Step 1 of 5</p>
							<h3
								id="guided-stage-heading"
								bind:this={stageHeading}
								tabindex="-1"
								data-testid="barnum-stage-heading"
							>
								First impression
							</h3>
							<span
								>Read each line once and answer with your first impression. Every rating is
								optional.</span
							>
						</div>
						{#if baselineHistory.length}
							<section
								class="baseline-history"
								data-testid="barnum-baseline-history"
								aria-labelledby="baseline-history-heading"
							>
								<h4 id="baseline-history-heading">Earlier lines</h4>
								<ol>
									{#each baselineHistory as statement (statement.id)}
										<li>
											<p>{statement.text}</p>
											<span>{surfaceRatingLabel(statement.rating)}</span>
										</li>
									{/each}
								</ol>
							</section>
						{/if}
						{#if currentBaselineStatement}
							<div
								class="current-baseline"
								bind:this={currentBaselineContainer}
								data-testid="barnum-current-baseline"
								tabindex="-1"
							>
								<ReadingCard
									statement={currentBaselineStatement}
									index={presentedBaselineStatements.length}
									label={`Line ${presentedBaselineStatements.length} of ${baselineCore.length}`}
									onrate={(rating) => rateStatement(currentBaselineStatement.id, rating)}
								/>
							</div>
						{/if}
						<p class="method-note">
							Ratings are optional. Choose “Too broad to tell” whenever a line feels impossible to
							judge fairly.
						</p>
					{:else if stage === 'four-clues'}
						<div class="stage-heading">
							...
							<span
								>All four are optional. Choose only what you are comfortable sharing in this tab.</span
							>
						</div>
						<div class="clue-grid">
					...
					{:else if stage === 'apparent-sharpening'}
						<div class="stage-heading">
							...
							<span
								>Read the whole profile once, then rate the reading as a whole if you want to.</span
							>
						</div>
						<div class="full-reading" data-testid="barnum-full-reading">
							{#each fullReadingStatements as statement, index (statement.id)}
								<ReadingCard {statement} index={index + 1} showRating={false} mode="plain" />
							{/each}
						</div>
						<fieldset class="whole-rating" data-testid="barnum-whole-reading-rating">
							<legend>Overall, how personally fitting does this reading feel?</legend>
							<div>
								{#each SIX_POINT_VALUES as value (value)}
									<label>
										<input
											...
											aria-label={`${value} of 5${value === 0 ? ', not fitting' : value === 5 ? ', very fitting' : ''}`}
											onchange={() => setWholeReadingFit(value)}
										/>
										<span>{value}</span>
										<small>{value === 0 ? 'Not fitting' : value === 5 ? 'Very fitting' : ''}</small>
									</label>
								{/each}
							</div>
						</fieldset>
					{:else if stage === 'feedback-and-counterfactual'}
						<div class="stage-heading">
							...
							<span>Here is one short final pass. The explanation comes next.</span>
						</div>
							...
							<div class="reading-grid two-columns">
								{#each feedbackStatements as statement, index (statement.id)}
									<ReadingCard
										{statement}
										index={index + 1}
										label={`Reading ${String.fromCharCode(65 + index)}`}
										mode="plain"
										showRating={false}
									/>
								{/each}
						...
						{:else}
							<p class="method-note">
								You did not mark a line as a fit, so no new line was built from your ratings.
							</p>
						{/if}
						<button
							class="counterfactual-button"
							type="button"
							data-testid="barnum-guided-counterfactual"
							disabled={counterfactualApplied}
							onclick={applyCounterfactual}
						>
							{counterfactualApplied ? 'Surface clues changed' : 'Change several surface clues'}
						</button>
					...
					{:else if stage === 'reveal'}
						<div class="stage-heading reveal-heading">
							...
							<ul class="reveal-summary" data-testid="barnum-reveal-summary">
								{#each revealSummary as line (line)}<li>{line}</li>{/each}
							</ul>
						</div>
						...
						<section id="barnum-xrays" class="x-rays" aria-labelledby="xray-heading">
							<h4 id="xray-heading">Where the shown lines came from</h4>
							{#if xrayStatements.length}
								<div class="reading-grid">
									{#each xrayStatements as statement, index (statement.id)}
										<ReadingCard
											{statement}
											index={index + 1}
											label={xrayCardLabel(statement, index)}
											mode="xray"
											showRating={false}
										/>
									{/each}
							...
							{:else}
								<p class="method-note">No reading line was shown before the method was revealed.</p>
							{/if}
						</section>
						...
						<div class="reveal-actions">
							...
							<button type="button" data-testid="barnum-replay-same-deck" onclick={replaySameDeck}>
						...
						<footer class="guided-actions">
							...
							<div class="persistent-actions">
								...
								<button
									class="primary"
									type="button"
									data-testid={stage === 'baseline' && !allBaselinePresented
										? 'barnum-next-statement'
										: 'barnum-continue'}
									data-guided-action={stage === 'baseline' && !allBaselinePresented
										? 'next-statement'
										: 'continue'}
			...
			<noscript>
				<section class="noscript-poster">
					...
					<p>
						A warm, broad statement can feel personal while fitting many people. A long reading also
						creates more chances to notice a match and forget a miss. The interactive demonstration
						needs JavaScript, but the article still explains the method, probability example,
						limitations, practical checklist, and research.
					</p>
					<p>
		...
		{#if stage === 'reveal' || stage === 'open-lab'}
			<section class="print-only" aria-labelledby="barnum-print-heading">
				<div class="print-summary">
					...
					<dl>
						...
						<div>
							<dt>Reviewed generic sentences</dt>
							<dd>{SURFACE_SENTENCE_COUNT}</dd>
						</div>
						<div>
							<dt>Reviewed feedback sentences</dt>
							<dd>{FEEDBACK_SENTENCE_COUNT}</dd>
						</div>
						<div>
							<dt>Reviewed direct echoes</dt>
							<dd>{DIRECT_ECHO_SENTENCE_COUNT}</dd>
						</div>
						<div>
							<dt>Manifest</dt>
					...
					<p>
						No visitor selections, context values, ratings, readings, or replay seed are printed.
					</p>
				</div>
				<DefensiveFieldGuide
...
<style>
	...
	.current-baseline:focus-visible {
		outline: 3px solid var(--barnum-focus);
		outline-offset: 3px;
	}
	...
	.baseline-history h4 {
		font: 760 0.72rem/1.3 var(--barnum-sans);
	}
	...
	.baseline-history p {
		font: 0.78rem/1.45 var(--barnum-serif);
	}

	.baseline-history span {
		color: var(--barnum-muted);
		font: 700 0.68rem/1.35 var(--barnum-sans);
		white-space: nowrap;
	}
	...
	.whole-rating label small {
		min-height: 1.2em;
		color: var(--barnum-muted);
		font: 0.65rem/1.2 var(--barnum-sans);
	}
	...
	.reveal-summary {
		...
		padding-left: 1.15rem;
		color: var(--barnum-muted);
		font: 0.76rem/1.5 var(--barnum-sans);
	}
	...
	.secondary-actions > div {
		...
		background: var(--barnum-raised);
		padding: 0.55rem;
		box-shadow: 0 0.5rem 1.25rem color-mix(in oklab, var(--barnum-ink) 12%, transparent);
	}
	...
	:global(.barnum-lab[data-ready='false'] .lab-scroll) {
		position: relative;
	}

	:global(.barnum-lab[data-ready='false'] .lab-scroll > :not(noscript)) {
		visibility: hidden;
	}

	:global(.barnum-lab[data-ready='false'] .noscript-poster) {
		position: absolute;
		z-index: 1;
		inset: 0;
		visibility: visible;
	...
	@container barnum-lab (max-width: 46rem) {
		...
		.guided-actions > .persistent-actions {
			display: grid;
			grid-template-columns: repeat(2, minmax(0, 1fr));
		}
	}
	...
	@media (max-width: 640px) and (min-height: 28rem) {
		.guided-actions {
			...
			padding: 0.4rem 0.65rem max(0.4rem, env(safe-area-inset-bottom));
		}
	}
